...
export class HistoryPage {
  ...
  request: string; //string to store request to server

  constructor(public navCtrl: NavController, private http: HttpClient, private ref: ChangeDetectorRef) {

    if (localStorage.getItem("ClassType") == "org.hawkoin.network.Vendor") { //sets request if user is a vendor
    ...
    else { //sets request if user is a spender
    ...
    this.http.get(this.request, httpOptions).subscribe((response) => { //reguests list from Fabric
      ...
      ref.detectChanges(); //forces angular to detect changes made to list
